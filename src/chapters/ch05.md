...
**Practice-Based Question (PBQ):** [Emma Woodhouse Handles a Hostile Offboarding](https://brendanpshea.github.io/securityx/problems/ch05-emma-iam-offboarding.html)
...
1. Federation lets the Service Provider read the user's password directly from the Identity Provider so it can verify it independently.
2. Deprovisioning a departing employee's access only in the HR system is sufficient if the HR system is the authoritative source.
3. RBAC can suffer from "role explosion" as organizations create increasingly specific roles to capture business reality.
4. ABAC decisions can incorporate attributes about the subject, the object, the action, and the environment.
5. Mandatory Access Control (MAC) lets the resource owner override classification decisions when convenient.
6. A Policy Decision Point evaluates a request and returns permit or deny; a Policy Enforcement Point implements that decision.
7. Conditional access typically combines signals such as device posture, geographic location, and time of day.
8. A leaked biometric template is no different from a leaked password — you simply enroll a new biometric.
9. Just-in-time access reduces standing privilege by elevating a user only for a defined window of time.
10. Hardcoded secrets in a git repository are safe to remove with a single delete commit because the history is inaccessible.
11. The Golden SAML attack works by stealing the SAML token-signing key from the federation server.
12. SAML troubleshooting frequently involves clock skew, certificate expiry, and entity ID mismatches.
13. OAuth 2.0 and OpenID Connect are the same protocol with two different names.
14. Kerberos depends critically on time synchronization between the client, server, and key distribution center.
15. 802.1X with EAP-TLS authenticates clients using certificates rather than passwords.
16. WPA3's SAE handshake is designed to resist offline dictionary attacks compared with WPA2's four-way handshake.
17. MFA fatigue attacks are best mitigated by disabling MFA for users who report frequent push prompts.
18. A PAM platform's value comes primarily from storing privileged passwords; session recording is unnecessary overhead.
19. Service accounts and workload identities should be subject to the same lifecycle discipline as human accounts.
20. Federation concentrates trust in cryptographic secrets such as token-signing certificates, which must be protected as Tier 0 assets.
21. Credential dumping is purely an endpoint malware topic and has little relevance to IAM architecture.
22. Lateral movement in enterprise environments often depends on the reuse of compromised identity material across trust zones.
23. Registering a new MFA device, adding a new OAuth consent grant, or disabling audit collection can all be examples of defensive evasion in identity systems.
24. LDAP injection, claim manipulation, and provisioning abuse are examples of trust-boundary failures that can affect identity workflows.

#### Answer Key

1. **False:** The SP never sees the user's password. The IdP authenticates the user and issues a signed assertion that the SP validates.
2. **False:** Deprovisioning must propagate to every downstream application; unfederated apps require SCIM, manual offboarding, or retirement.
3. **True:** Without disciplined role design, RBAC environments accumulate thousands of overlapping roles.
4. **True:** That is precisely the four-tuple ABAC evaluates.
5. **False:** MAC is enforced by the system regardless of owner preference — that rigidity is its security value.
6. **True:** The PDP/PEP separation is the standard architecture for centralized policy with distributed enforcement.
7. **True:** These context signals are the foundation of conditional access policies.
8. **False:** Biometrics cannot be reissued. A leaked template compromises that biometric forever.
9. **True:** JIT access is the cornerstone of modern PAM design.
10. **False:** Git history retains the secret. Rotation is the only safe response after exposure.
11. **True:** That is the core mechanism — once the signing key is stolen, the attacker can forge arbitrary assertions.
12. **True:** These are the three most common SAML failure modes.
13. **False:** OAuth 2.0 is an authorization framework. OIDC is an authentication layer built on top of it.
14. **True:** Kerberos typically requires time synchronization within five minutes.
15. **True:** EAP-TLS uses mutual certificate authentication.
16. **True:** SAE provides forward secrecy and resists offline dictionary attacks.
17. **False:** The right response is number matching, context-rich prompts, or phishing-resistant factors — not disabling MFA.
18. **False:** Session recording, approval workflows, and JIT elevation are all core PAM capabilities, not optional extras.
19. **True:** Service accounts often have broader privilege than humans and require equally rigorous lifecycle management.
20. **True:** The Golden SAML attack and many similar federation compromises hinge on the protection of these signing keys.
21. **False:** Credential dumping is an endpoint technique, but in SecurityX it is highly relevant to IAM because the stolen material is used to impersonate subjects, escalate privilege, and pivot across identity boundaries.
22. **True:** Reused Kerberos tickets, hashes, tokens, and over-permissioned service accounts are common mechanisms for identity-enabled lateral movement.
23. **True:** Quiet changes to MFA enrollment, OAuth grants, and logging are classic ways attackers evade detection while preserving identity-based access.
24. **True:** These attacks manipulate the trust logic of directory, federation, or provisioning systems rather than simply breaking a generic application field.
