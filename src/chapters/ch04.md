...
**Practice-Based Question (PBQ):** [Prospero Redesigns the Milan Island Network](https://brendanpshea.github.io/securityx/problems/ch04-prospero-zero-trust.html)
...
1. A WAF is most effective when used to protect public-facing applications and API entry points that accept untrusted input.
2. Microsegmentation is designed to reduce lateral movement by allowing architects to control communication between specific workloads or services.
3. Horizontal scaling generally improves fault tolerance compared with relying on a single vertically scaled system.
4. Network Access Control (NAC) can help restrict unmanaged or noncompliant devices before they receive normal network access.
5. A CDN can absorb some denial-of-service traffic and shield origin servers, but it does not eliminate the need to secure the origin environment.
6. Tagging strategies are useful because machine-readable metadata can drive automated handling and protection rules.
7. API gateways are important because APIs are trust boundaries that require validation, authorization, and logging.
8. A mature security metric should ideally say something about risk movement, not just event volume.
9. Device attestation helps prove that a system is in an expected and trustworthy state before or during access.
10. A zero-trust-inspired architecture may still use a VPN, but the VPN should be treated as transport rather than automatic trust.

**Scenario Multiple Choice**

11. A retailer's auditor sees that the company has both an IDS and an IPS deployed. The architect explains they serve different operational risk profiles. Which statement best captures the difference?
    a. IDS is inline and blocks traffic; IPS is out-of-band and only alerts.
    b. IDS is out-of-band and alerts on suspicious traffic; IPS is inline and can drop traffic, carrying greater risk of disrupting production if mis-tuned.
    c. They are functionally identical, and "IPS" is just a marketing rename of IDS.
    d. IPS only inspects encrypted traffic; IDS only inspects plaintext.

12. An employee on a managed laptop authenticates to the corporate VPN from a coffee shop. Two hours later, the same session begins issuing rapid administrative API calls from a previously unseen geography while the laptop's posture check now reports a disabled EDR agent. What does Zero Trust expect the architecture to do?
    a. Trust the session, because the user already authenticated through the VPN.
    b. Reauthenticate based on context: posture has degraded and risk signals have changed, so step up verification or revoke access.
    c. Wait until the session expires naturally before reevaluating trust.
    d. Block all VPN users globally until the incident is investigated.

13. A SaaS provider's DLP program inspects email attachments at the corporate gateway and nothing else. Six months later, sensitive customer data is exfiltrated through an employee's personal cloud storage account synced via the browser. Which architectural lesson best explains the failure?
    a. The email DLP signatures were out of date.
    b. DLP must follow the data across all relevant layers — endpoint, web/CASB, cloud SaaS, and storage — not just the email perimeter.
    c. The employee should have used email instead of cloud sync.
    d. DLP is fundamentally ineffective and should be replaced with encryption alone.

14. A regional bank operates a single primary data center with a hot-standby site 30 miles away. After a regional power failure takes both sites offline simultaneously, the bank's online services collapse. Which design principle was violated?
    a. Horizontal scaling — they should have added more servers in the primary site.
    b. Geographic and dependency diversity — secondary sites must not share the same fate-sharing failure domain (power grid, fiber path, region).
    c. Zero Trust — they trusted users by network location.
    d. Defense-in-depth — they did not deploy enough firewalls.

15. A subject-object access decision is failing intermittently for a microservice calling an internal API. Which architectural component is the right place to enforce, validate, and log the relationship between the calling subject and the requested object?
    a. The reverse proxy in front of the customer-facing web app.
    b. The Web Application Firewall, because it understands HTTP.
    c. The API gateway, which authenticates the caller, validates the request, and logs the subject-object access for the protected service.
    d. The Content Delivery Network, because it is closest to the user.

#### Answer Key

1. **True:** WAFs are designed to understand and defend web-layer interactions where untrusted user input reaches the application.
2. **True:** Microsegmentation narrows allowed communication paths and limits lateral movement opportunities.
3. **True:** Distributing workload across multiple nodes generally reduces single points of failure.
4. **True:** NAC is valuable at the onboarding edge because it can condition access on device identity or posture.
5. **True:** CDNs provide shielding and edge capacity, but the origin still requires sound security architecture.
6. **True:** Tags and labels help systems automatically apply policy rather than relying on human memory.
7. **True:** APIs are machine-to-machine trust boundaries and require explicit validation and control.
8. **True:** Good security metrics help leadership see whether risk is improving, stagnating, or worsening.
9. **True:** Attestation provides evidence that a device or workload has booted and operates in an expected state.
10. **True:** Zero Trust does not ban VPN usage; it changes what a VPN connection means.
11. **b. IDS out-of-band, IPS inline.** IPS can drop traffic, so misconfiguration is felt by production immediately; IDS observes and alerts without affecting flow.
12. **b. Reauthenticate based on context.** Continuous authorization means access is reevaluated when risk signals change — geography, posture, or behavior — not held constant just because the initial login succeeded.
13. **b. DLP must follow the data across layers.** A perimeter-only DLP misses the channels people actually use. Modern DLP requires endpoint agents, CASB/web inspection, SaaS-native controls, and storage-layer policy.
14. **b. Geographic and dependency diversity.** A 30-mile failover site shares the same regional power grid and likely the same fiber path. True resilience requires breaking fate-sharing across power, network, and region.
15. **c. The API gateway.** API gateways are designed to authenticate the calling subject, validate the request against allowed objects/methods, enforce rate and scope limits, and log the access decision — making them the natural enforcement point for subject-object relationships in service-to-service traffic.