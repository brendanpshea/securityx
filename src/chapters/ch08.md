...
> The Mad Hatter, lead cryptographer at Teatime Tech, takes a frantic call at 3:17 a.m. The company's flagship API has been refusing connections for forty-five minutes. Customers are escalating; the on-call engineer cannot find a smoking gun in the application logs. The Mad Hatter looks at the TLS chain and exhales — the *intermediate* CA the company had been using for nine years quietly expired at midnight. The leaf certificate is fine. The root in everyone's trust store is fine. The link in the middle is not, and TLS chain validation is failing on every modern client. The fix takes forty seconds: deploy a new chain that includes the *re-issued* intermediate. The post-mortem takes two weeks and produces a new policy: every certificate in the company's hierarchy is monitored by an automated expiration tracker, alerts fire 90, 60, 30, 14, and 3 days out, and intermediate-CA renewals are treated with the same rigor as production deployments.
...
> In 2011, the Dutch certificate authority **DigiNotar** was compromised. Attackers — believed to be Iranian state-aligned — gained access to DigiNotar's issuing infrastructure and produced more than five hundred fraudulent certificates, including a wildcard for `*.google.com`. The bogus certificates were used to mount man-in-the-middle attacks against approximately 300,000 Iranian Gmail users. When Google and Mozilla investigated, they found logging gaps so severe that DigiNotar could not confidently say which certificates were fraudulent and which were legitimate. Within weeks, every major browser revoked trust in the entire DigiNotar root, including certificates that had been legitimately issued. DigiNotar declared bankruptcy. Two consequences shaped modern PKI: **Certificate Transparency** logs were created so unauthorized issuance can be detected publicly, and browsers became dramatically less tolerant of CA operational failures.
