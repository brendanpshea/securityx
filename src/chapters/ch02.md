...
<p class="pbq-promo-title"><a href="https://brendanpshea.github.io/securityx/problems/ch02-darcy-risk-modeling.html">Fitzwilliam Darcy Quantifies a Vendor Risk</a></p>
<p class="pbq-promo-text">Design PBQ. Build out Darcy's board memo from a vendor risk register entry — compute SLE and ALE, classify the threat actor, pick a framework, and recommend a treatment strategy.</p>
</div>
</div>
...
<p class="pbq-promo-title"><a href="https://brendanpshea.github.io/securityx/problems/ch02-darcy-incident.html">Darcy Reviews the Pemberley Loss</a></p>
<p class="pbq-promo-text">Diagnostic PBQ. Eight months later the breach Darcy modeled actually happens — and the realized loss is 4.8× the SLE. Walk backward through the loss report and name the specific risk-management control (including the impact model itself) that failed.</p>
</div>
</div>
...
Talk to someone who actually does risk work — a risk manager, threat modeler, GRC director, business continuity manager, internal auditor, or cyber-insurance broker. A few questions worth asking, in roughly this order:

- How do you decide what goes on the risk register and what gets left off?
- Walk me through the last time the business overruled a risk recommendation. Why did they overrule it, and were they right in hindsight?
- How do you talk about cyber risk to people who don't think in probabilities?
- What is a risk you have learned to live with that would horrify a textbook?
- Where does threat modeling actually happen in your software development lifecycle, and where does it not happen?

Listen for the gap between risk on paper — heat maps, ALE calculations, frameworks — and the way real decisions get made under uncertainty and budget pressure.

> *No one to interview?* Run the conversation with a chatbot in role. A prompt that works: *"You are the head of enterprise risk at a mid-sized regional bank. You have ten years of experience and report to the Chief Risk Officer. You have lived through one major incident and two near-misses. I'm interviewing you for a class. Answer in character. When I ask something a real risk lead would not actually know, say so. Push back on questions that are too theoretical."* The rehearsal exposes which of your questions are too abstract to provoke a real answer.

#### Hold a Debate *(1 hour)*

Pick the position you find *least* sympathetic and argue it as well as you can:

- Quantitative risk analysis (ALE, FAIR) is the only honest way to compare cyber risk against other business risks. Qualitative heat maps are theater that let leadership avoid hard prioritization decisions.
- Qualitative analysis is more honest because it does not disguise guesses as math. False precision is more dangerous than acknowledged uncertainty when the underlying probabilities cannot be reliably estimated.
- The whole framing is wrong. Risk registers and heat maps exist to defend security budget to executives, not to make the organization safer. The right unit of analysis is the threat model of a specific system, not the organization's "top ten risks."
...
> *Studying alone?* Sketch your argument, then ask a chatbot to attack it from the opposite position. A prompt that produces real sparring: *"Take the opposite position from the argument below and attack it as aggressively as a senior CRO who deeply disagrees. Cite real incidents, real failed risk programs, and real analyst-vs-executive dynamics. Don't be polite. After each of my replies, escalate."* The goal isn't to beat the model. It's to find out which parts of your case fold under pressure.

#### Track a Current Debate *(20 min)*

Find a risk-relevant event from the last six months. Good hunting grounds: the annual Verizon Data Breach Investigations Report, CISA advisories on widely exploited vulnerabilities, ransomware-tracking sites that aggregate leak-site activity, FAIR Institute publications, NIST SP 800-30 and SP 800-37 updates, and court rulings on negligence or duty of care in cybersecurity. Be ready to explain to a classmate, in two minutes, what happened, which risk treatment (mitigation, transference, avoidance, acceptance) the affected organization had implicitly chosen, and what other organizations are reconsidering as a result. Bring two primary sources you'd point them to if they wanted to read further.

#### Lab Artifact: STRIDE a Real Feature *(20 min)*

The hardest part of threat modeling is choosing a small enough target to actually finish. In this activity you will STRIDE-decompose a single feature of a real public web application — using only its public documentation and your own observation.

1. Pick a single feature you actually use. Good targets: a "Sign in with Google" OAuth flow, GitHub repository sharing with an external collaborator, a bank's "transfer to external account" flow, the meeting-link join flow for a video conferencing service.
2. From the public documentation and what you can see from outside the system, sketch a simple data flow: actor → trust boundary → component → data store. Hand-drawn or in any diagramming tool is fine.
3. Walk through STRIDE for that feature. For each letter — Spoofing, Tampering, Repudiation, Information Disclosure, Denial of Service, Elevation of Privilege — name one plausible threat (or note "not applicable here, because…").
4. Pick the three threats you find most credible. For each, name one control that already appears to exist and one control that you cannot tell from the outside whether it exists.

**Safety:** This activity is based entirely on public documentation and observation. Do not probe, scan, or attempt to test the system. The goal is structured reasoning about a real target, not pentesting.

#### Walk a Tabletop *(2+ hours)*

Pick one scenario and walk through it phase by phase, naming what you would do, what evidence you would collect, who you would call, and what would have to be true *in advance* for the response to actually work. The point of a tabletop is to find what is missing — the call tree nobody updated, the runbook that assumes a tool you don't have, the backup nobody has tested restoring from.

- A ransomware group claims they have your customer database. They publish a sample row that contains a real customer's real data. They demand $5 million in 48 hours and threaten to publish the rest.
- Your primary cloud region goes dark at 3 a.m. The provider's status page shows a major incident with no ETA. Your hot-standby in another region was last validated six months ago. The CEO wants an update at 4 a.m.
- An insider with sysadmin access has resigned, given two weeks' notice, and your DLP system has flagged that they appear to be copying repositories to a personal device. Their last day is Friday.

Walk: preparation (what should already be true), detection and analysis, containment, eradication and recovery, post-incident activity. At each phase, write down what is missing.

> *No group to run the tabletop?* Ask a chatbot to facilitate. A prompt that works: *"You are facilitating a tabletop exercise. I am the incident commander. Walk me through the scenario below one phase at a time. After my decisions in each phase, inject one realistic complication a real incident would throw at me — a stakeholder calling, a tool going down, a piece of evidence contradicting an earlier assumption. Don't let me skip steps."* The model is bad at being a responder. It is unusually good at being an annoying facilitator, which is what a tabletop needs.
